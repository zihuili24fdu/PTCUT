...
# CycleGAN测试脚本 - GNB 2-Class 节细胞神经母细胞瘤分类 (Baseline)
# 
...
#
# 使用方法:
#   bash test_cyclegan_gnb.sh [mode]
# 模式:
#   test (默认): 仅运行测试生成图像
#   eval: 仅对已有的测试结果进行评估
...
echo "================================================================"
echo "CycleGAN测试 (Baseline) - GNB 2-Class 节细胞神经母细胞瘤"
echo "================================================================"
echo "运行模式: $MODE"
echo "数据集目录: $DATAROOT"
echo "实验名称: $NAME"
...
if [ "$RUN_TEST" = true ]; then
    ...
    if [ ! -d "$CHECKPOINT_DIR" ]; then
        ...
        echo "请先运行训练脚本: bash train_cyclegan.sh"
        exit 1
    fi

    if [ "$EPOCH" = "latest" ]; then
        CHECKPOINT_FILE="$CHECKPOINT_DIR/latest_net_G_A.pth"
    else
        CHECKPOINT_FILE="$CHECKPOINT_DIR/${EPOCH}_net_G_A.pth"
    fi

    if [ ! -f "$CHECKPOINT_FILE" ]; then
        echo "❌ 错误: Checkpoint文件不存在: $CHECKPOINT_FILE"
        echo "可用的checkpoints:"
        ls -1 "$CHECKPOINT_DIR"/*_net_G_A.pth 2>/dev/null || echo "  (无)"
        exit 1
    fi
